...
export default {
	...
	install(Vue, options) {
		Vue.http.interceptors.push((request, next) => {
			...
			if (token && !hasAuthHeader) this.setAuthHeader(request);

			next((response) => {
				if (response.url === REFRESH_TOKEN_URL && this._isInvalidToken(response)) return this.logout('SESSION_EXPIRED');
	...
	login(context, creds, redirect) {
		...
		return Vue.http.post(LOGIN_URL, JSON.stringify(params), AUTH_BASIC_HEADERS).then((response) => {
			...
			// this._setAxiosToken();
			if (redirect) context.$router.push('/');
	...
	setAuthHeader(request) {
		if (!request.authOut) request.headers.set('Authorization', 'Bearer ' + store.getters.auth.accessToken);
	},
	...
	_refreshToken(request) {
		const params = {'grant_type': 'refresh_token', 'refresh_token': store.getters.auth.refreshToken};

		return Vue.http.post(REFRESH_TOKEN_URL, JSON.stringify(params), AUTH_BASIC_HEADERS).then((result) => {
			this._storeToken(result);
			// this._setAxiosToken();
			if (request) return this._retry(request);
		}).catch((errorResponse) => {
